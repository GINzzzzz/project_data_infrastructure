...
- уникальность объекта / маршрута / мероприятия - три сущности должны иметь УИН в рамках всей системы хранения (так как они повторяются на 2 уровнях и между проектами): т.е. 2 УИНа - id строки в рамках таблицы    данных (автоматически через serial) и УИН строки в рамках системы, назначаемой полуавтоматически (на стадии загрузки данных к индексу добавляются системные идентификаторы БД и таблицы). ЭТОТ НОМЕР НЕ ДОЛЖЕН 
  МЕНЯТЬСЯ НИКОГДА, ПОКА ОБЪЕКТ / МАРШРУТ / МЕРОПРИЯТИЕ ХРАНЯТСЯ;
